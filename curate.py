...
def export_gallery_json() -> int:
    ...
    with db_session(DB_PATH) as conn:
        ...
        cursor.execute(
            ...
            SELECT id, category, width, height, format, filesize,
                   aspect_ratio, orientation, s3_url, s3_thumb_url, s3_png_url, s3_jpg_url,
                   curated_filename, source
            FROM wallpapers
    ...
    for r in rows:
        ...
        w = {
            ...
            "png_url": r["s3_png_url"] or r["s3_url"],
            "jpg_url": r["s3_jpg_url"] or r["s3_url"],
            "width": r["width"],
            "height": r["height"],
...
def run_cdn_publish_task(tm, force_all: bool = False) -> dict:
    ...
    from curate_config import GIT_BRANCH, GIT_CONFIRM_TOKEN, GIT_MAX_PUSH_MIN, GIT_REMOTE
    from curate_s3 import is_s3_configured, sync_curated_collection, sync_multi_formats, sync_thumbnails
    ...
    # 1. Sync full-res + multi-formats + thumbnails to B2/S3 (skip gracefully when not configured)
    sync_result = {}
    if is_s3_configured():
        def _progress(done, total, msg):
            tm.set_progress(done, total, current_name=msg)
        tm.log("📤 Uploading unsynced curated wallpapers & multi-formats (PNG+JPG) to B2/S3...")
        try:
            ...
            # Sync any missing multi-formats (PNG + JPG)
            fmt_res = sync_multi_formats(
                force_all=force_all,
                progress_callback=_progress,
                curated_dir=CURATED_DIR,
                db_path=DB_PATH,
            )
            if fmt_res.get("uploaded", 0) > 0:
                tm.log(f"📦 Multi-formats synced: {fmt_res['uploaded']} wallpapers converted & uploaded as PNG+JPG")
            ...
            if thumb_res.get("uploaded", 0) > 0:
                tm.log(f"🖼️ Thumbnails synced: {thumb_res['uploaded']} WebP thumbs uploaded")
